...
def avg_pool_backward_reshape(dout, cache):
    ...
    dx_reshaped = np.zeros_like(x_reshaped)
    out_newaxis = out[:, :, :, np.newaxis, :, np.newaxis]
    mask = np.ones_like(x_reshaped) / (x_reshaped.shape[3] * x_reshaped.shape[5])
    dout_newaxis = dout[:, :, :, np.newaxis, :, np.newaxis]
    dout_broadcast, _ = np.broadcast_arrays(dout_newaxis, dx_reshaped)
...
def avg_pool_forward_im2col(x, pool_param):
    """
    An implementation of the forward pass for average pooling based on im2col.
    """
    N, C, H, W = x.shape  
    pool_height, pool_width = pool_param["pool_height"], pool_param["pool_width"]
    stride = pool_param["stride"]
...
def avg_pool_backward_im2col(dout, cache):
    """
    An implementation of the backward pass for average pooling based on im2col.
    """
    ...
    for i in range(dx_cols.shape[1]):
        dx_cols[:, i] = dout_reshaped[i] / pool_size
    dx = col2im_indices(
        dx_cols, (N * C, 1, H, W), pool_height, pool_width, padding=0, stride=stride  
    )
    dx = dx.reshape(x.shape)
